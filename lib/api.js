  ...
  api.mediaScans = {
    ...
    addFiles: function(scan, data) {
      return api({
        ...
        body: data
      }).then(createdHandler(data, 'add files to media scan'));
    }
  };
  ...
  api.mediaFiles = {
    find: function(query, options) {
      return api(_.extend({}, options, {
        url: '/media/files',
        qs: _.extend({}, query)
      })).then(retrievedHandler('find media files matching ' + JSON.stringify(_.extend({}, query)), options));
    },
  ...
  api.mediaSearches = {
    findAll: function(query, options) {
      return api.all(_.extend({}, options, {
        url: '/media/searches',
        qs: _.extend({}, query)
      })).then(retrievedHandler('find media searches matching ' + JSON.stringify(_.extend({}, query))));
    }
  };
