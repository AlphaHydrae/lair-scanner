...
class SourceScan {
  constructor(source, scanner, options) {
    ...
    this.logger = scanner.config.logger('source-scan');
  ...
  startProgress() {
    if (this.filesToDownload >= 1 && this.firstLevelFilesToScan >= 1) {
      this.progress = 0;
  ...
  tickProgress(n) {
    ...
    if (this.progressBar) {
      this.progressBar.tick(n, {
        file: this.currentFile,
        dl: Math.round(this.filesDownloaded * 100 / this.filesToDownload),
        scan: Math.round(this.firstLevelFilesScanned * 100 / this.firstLevelFilesToScan)
      });
    }
  }
  ...
  processChanges() {
    ...
    promise = promise.then(() => this.streamChangedFiles(file => {
      ...
      if (this.options.list && (file.change != 'identical' || this.options.listIdentical)) {
        this.logger.info(this.getFileChangeMessage(file.change, file.path));
      }
    }));
