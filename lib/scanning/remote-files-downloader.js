...
class RemoteFilesDownloader extends EventEmitter {
  ...
  simplifyFile(file) {
    ...
    simplified.fileCreatedAt = moment(file.fileCreatedAt).startOf('second').toISOString();
    simplified.fileModifiedAt = moment(file.fileModifiedAt).startOf('second').toISOString();
    return simplified;
  }
}
